<?php
class ControllerCatalogSchooluser extends Controller
{
    private $error = array();
    ...
    {
        //echo '<pre>' . print_r($this->user->session->data['user_id'],true) . '</pre>';
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('catalog/schooluser', 'token=' . $this->session->data['token'] . $url, true)
        );

        $data['add'] = $this->url->link('catalog/schooluser/add', 'token=' . $this->session->data['token'] . $url, true);
        $data['copy'] = $this->url->link('catalog/schooluser/copy', 'token=' . $this->session->data['token'] . $url, true);
        $data['delete'] = $this->url->link('catalog/schooluser/delete', 'token=' . $this->session->data['token'] . $url, true);
        ...
        $filter_data = array(
            ...
            'filter_status'   => true,
            'filter_user' => $this->user->session->data['user_id'],
            'sort'            => $sort,
            'order'           => $order,
        ...
        foreach ($results as $result) {
            ...
            $regstatus = 'Tutup';
            $regStatusId = 3;
            if (isset($result['stock_status_id'])) {
                $regStatusId = $result['stock_status_id'];
                if ($result['stock_status_id'] == 1 && $result['date_closed'] && $result['date_closed'] <> '0000-00-00') {
                    $now = time();
                ...
                if ($regStatusId <> 4) {
                    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "stock_status WHERE stock_status_id ='" . (int)$regStatusId . "' AND language_id ='" . (int)$result['language_id'] . "' LIMIT 1");
            ...
            $data['products'][] = array(
                ...
                'edit'       => $this->url->link('catalog/schooluser/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, true),
                'view'       => HTTPS_CATALOG.'index.php?route=product/school&product_id=' . $result['product_id'],
        ...
        $data['sort_name'] = $this->url->link('catalog/schooluser', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, true);
        $data['sort_status'] = $this->url->link('catalog/schooluser', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, true);
        $data['sort_order'] = $this->url->link('catalog/schooluser', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, true);
        ...
        $pagination->url = $this->url->link('catalog/schooluser', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
        ...
        $this->response->setOutput($this->load->view('catalog/schooluser_list', $data));
    }
    ...
    public function edit()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $school = $this->db->query("SELECT `status`, user_id FROM " . DB_PREFIX . "product WHERE product_id ='" . (int)$this->request->get['product_id'] . "' LIMIT 1" );
            $this->request->post['status'] = $school->row['status'];
            $this->request->post['user_id'] = $school->row['user_id'];
            ...
            $this->response->redirect($this->url->link('catalog/schooluser', 'token=' . $this->session->data['token'] . $url, true));
        }
    ...
    public function getForm()
    {
        ...
        if (!isset($this->request->get['product_id'])) {
            $data['action'] = $this->url->link('catalog/schooluser/add', 'token=' . $this->session->data['token'] . $url, true);
        } else {
            $data['action'] = $this->url->link('catalog/schooluser/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, true);
        }

        $data['cancel'] = $this->url->link('catalog/schooluser', 'token=' . $this->session->data['token'] . $url, true);
        $this->load->model('tool/image');

        if (isset($this->request->get['product_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $product_info = $this->model_catalog_school->getSchool($this->request->get['product_id']);
        }
        $categories = $this->model_catalog_school->getProductCategories($this->request->get['product_id']);
        $categoryId = $categories[0];
        $this->load->model('tool/image');
        $edu = $this->db->query("SELECT * FROM " . DB_PREFIX . "education_category WHERE category_id = '" . (int)$categoryId . "' LIMIT 1");
        ...
        // die;
        $this->response->setOutput($this->load->view('catalog/schooluser_form', $data));
    }
