...
class ControllerCatalogFilter extends Controller {
	...
	public function kota() {
		/*$kota = $this->db->query("SELECT * FROM wilayah_2020 WHERE LENGTH(kode) = 5");
		$groupId = 6;
		$lastFilterId = 18;
		$sort = 0;
		foreach ($kota->rows as $k) {
			$lastFilterId++;
			$sort++;
			$sql1 = "INSERT INTO oc_filter (filter_group_id, sort_order) VALUES (" . $groupId . ", " . $sort .")";
			$sql2 = "INSERT INTO oc_filter_description VALUES (" . $lastFilterId . ", 1, " . $groupId . ", " . " '" . $k['nama'] . "')";
			$this->db->query($sql1);
			$this->db->query($sql2);
		}*/
		$kota = $this->db->query("SELECT * FROM oc_filter_description WHERE filter_id >= 19");
		foreach ($kota->rows as $k) {
			$sql2 = "INSERT INTO oc_filter_description VALUES (" . $k['filter_id'] . ", 2, " . $k['filter_group_id'] . ", " . " '" . $k['name'] . "')";
			$this->db->query($sql2);
